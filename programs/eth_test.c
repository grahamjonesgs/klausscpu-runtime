...
static uint16_t mdio_read(uint32_t phy_addr, uint32_t reg_addr) {
    for (int i = 0; i < 32; i++) mdio_clock(1, 1);   /* preamble */
...
static int test_mdio_phy_id(void) {
    ...
    REG_ETH_PHY_RESET = 1;  delay_ms(30);
    REG_ETH_PHY_RESET = 0;  delay_ms(120);
    ...
    for (uint32_t r = 0; r <= 6; r++) {
        printf("    reg 0x%02lx = 0x%04lx\n",
               (unsigned long)r, (unsigned long)mdio_read(0x01, r));
    }
...
static int test_phy_loopback(void) {
    ...
    REG_ETH_TX_START  = 1;
    printf("  TX: %lu-byte frame sent (etype 0x88B5), waiting for loopback RX...\n",
...
got_rx: {
        ...
        printf("  RX: %lu bytes in slot %lu\n", (unsigned long)len, (unsigned long)slot);
        if (len != sizeof(frame)) {
            printf("  length mismatch: got %lu, expected %lu\n",
                   (unsigned long)len, (unsigned long)sizeof(frame));
...
static void test_external_tx(void) {
    ...
    while (!REG_ETH_TX_READY) { }
    volatile uint8_t *tx = ETH_TX_SLOT(1);      /* use slot 1; keep slot 0 for loopback */
...
static void test_passive_rx(void) {
    ...
    while (1) {
        if (REG_ETH_RX_EV_PENDING & 1u) {
            ...
            REG_ETH_RX_EV_PENDING = 1;          /* W1C — release slot first */
            frame_count++;
            ...
            printf("  #%lu  len=%lu  "
                   ...
                   (unsigned long)len,
                   (unsigned long)rx[6],  (unsigned long)rx[7],
                   (unsigned long)rx[8],  (unsigned long)rx[9],
                   (unsigned long)rx[10], (unsigned long)rx[11],
                   (unsigned long)etype);
        }
    }
}
