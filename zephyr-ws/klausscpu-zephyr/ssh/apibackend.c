/*
 * apibackend.c — loadable backend for the HTTP control/telemetry API (/api/*).
 *
 * The kernel's webapi.c is only a dispatcher; this extension supplies the actual
 * route logic and registers it with webapi_register() at svc_start, removing it
 * at svc_stop.  Built as a single-source ELF-object llext (add_llext_target in
 * apps/ssh_shell/CMakeLists.txt) and loaded with `svc load /SD:/apibackend.llext`.
 *
 * The same handler serves BOTH the :80 (httpd) and :443 (httpsd) servers, so the
 * 7-seg software shadow (s_seg) is a single instance and stays consistent across
 * both ports.
 *
 * Symbols resolved at load: httpd_send + webapi_register/webapi_unregister
 * (kernel exports), snprintk + str* (ssh/llext_exports.c), k_uptime_get (Zephyr
 * builtin).  board_io.h is inline MMIO (compiled in).  No threads/sockets/fs, so
 * none of the whole-server extension's llext gotchas apply.
 */
...
/* The registered backend handler.  Same contract as webapi_handle for an /api
 * route (the dispatcher only calls us for "/api/..."). */
static bool api_handle(struct httpd_conn *c, const char *method,
